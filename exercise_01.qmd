...
What happens? 
I swear, I do not know what those symbols are. I tried looking it up and it said it was the option key, and I can guess the others are the up arrow and the letter K? But when I do that, all it generates are little degree symbols, so I imagine, I'm misunderstanding the symbols.

How can you get to the same place using the menus? 
Again, can't answer, but looking at the IDE cheatsheet shown in class, using mixtures of the arrow keys and control/command keys allows for either moving between desktop views and navigating lines. For layout commands that can toggle different panels, such as plots, packages, history, environment, or console, it is the control key and a numerical key.
